...
Q_Agent_2::Q_Agent_2(const Eigen::Ref<const Eigen::VectorXd> &action_vector_, BaseQuantisation *quant_class, BaseReward *reward_class) : Q_table(action_vector_.rows(), quant_class->getStateSize())
{
...
int Q_Agent_2::policy(int state) const
{
    return 0;
}

int Q_Agent_2::policy(const Eigen::Ref<const Eigen::VectorXd> &obs) const
{
...
void Q_Agent_2::load(fs::path agent_folder)
{
...
void Q_Agent_2::save(fs::path agent_folder) const
{
    std::string filename = agent_folder.append("q_matrix.csv").c_str();
...
std::vector<double> Q_Agent_2::train(int epochs, double alpha, double gamma, HaxBall &sim, int trajectory_length, Greediness* greediness)
{
    ...
    std::vector<int> actions_cur;
    std::vector<int> q_table_indices;
    std::vector<int> actions_next;
    std::vector<int> q_table_indices_next;
    ...
    for (int i = 0; i < epochs; ++i) // for loop over all epochs/trajectories
    {
        ...
        for (int j = 0; j < trajectory_length; ++j) // for loop over whole trajectory
        {
            ...
            q_table_index_next = getQtableStateIndex(state); // get row index for Q-table of next state
            ...
            rewards.push_back(reward(state, action_cur, state_next));
            actions_cur.push_back(action_cur);
            actions_next.push_back(action_next);
            q_table_indices.push_back(q_table_index);
            q_table_indices_next.push_back(q_table_index_next);


            // update value in Q-table
            if(rewards.size()==step_number)     // multi stepping is vector is full
            {
                rew_multi = get_multi_step_reward(rewards, step_number, gamma_multi);
                Q_table(actions_cur[0], q_table_indices[0]) = Q_table(actions_cur[0], q_table_indices[0]) + alpha * (rew_multi + gamma * Q_table(actions_next[0], q_table_indices_next[0]) - Q_table(actions_cur[0], q_table_indices[0]));
                rewards.erase(rewards.begin());
                actions_cur.erase(actions_cur.begin());
                actions_next.erase(actions_next.begin());
                q_table_indices.erase(q_table_indices.begin());
                q_table_indices_next.erase(q_table_indices_next.begin());
            }
            else {
...
int Q_Agent_2::getQtableStateIndex(const Eigen::Ref<const Eigen::VectorXd> &state) const
{
    return quant_class_->getQtableStateIndex(state);
}

double Q_Agent_2::reward(const Eigen::Ref<const Eigen::VectorXd> &o, int a, const Eigen::Ref<const Eigen::VectorXd> &o_prime) const
{
    return reward_class_->get_reward(o, a, o_prime);
}

std::string Q_Agent_2::getType() const
{
...
Eigen::VectorXd Q_Agent_2::getActionVector() const
{
    return action_vector;
}

void Q_Agent_2::printInfo() const
{
    std::cout << "TODO \n"; // @todo print info
}
...
int Q_Agent_2::getBestActionIndex(int stateIndex) const
{
...
int Q_Agent_2::getRandomActionIndex() const
{
...
double Q_Agent_2::get_multi_step_reward(std::vector<double> rewards, int step_number, double gamma_multi){
    double rew_multi = 0;
    double reward_tmp = 0;
